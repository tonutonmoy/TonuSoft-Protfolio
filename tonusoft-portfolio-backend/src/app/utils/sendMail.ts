...
class Email {
  ...
  constructor(user: User) {
    this.user = user;
    this.to = user.email || 'admin@gmail.com';
    this.from = `FLIND <${process.env.EMAIL_FROM || config.email_user}>`;
  }
  ...
  private newTransport() {
    ...
    return nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: process.env.MAILTRAP_USER || config.mail,
        pass: process.env.MAILTRAP_PASSWORD || config.mail_password,
      },
    } as any);
  }
...
// ============== Standalone Bulk Email Function ==============

export const sendBulkEmail = async (recipients: string[], html: string, subject: string) => {
  try {
    const transporter = nodemailer.createTransport({
      host: process.env.MAILTRAP_HOST || 'sandbox.smtp.mailtrap.io',
      port: Number(process.env.MAILTRAP_PORT) || 587,
